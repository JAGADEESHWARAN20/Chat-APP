...
// PATCH: Accept a join request or room switch notification
export async function PATCH(
  ...
  try {
    ...
    // Update the notification's status to 'read' and join_status to 'accepted'
    ...
    if (updateError) {
      ...
      console.log(`[Notifications Accept] Updated notification ${notificationId}: status to read, join_status to accepted`);
    }

    // Notify the requester that they were accepted
    const message = `Your request to ${notificationCore.type === "join_request" ? "join" : "switch to"} ${roomData.name} was accepted`;
    const { error: acceptNotificationError } = await supabase
      .from("notifications")
      .insert({
        user_id: notificationCore.sender_id,
        type: notificationCore.type === "join_request" ? "user_joined" : "room_switch_accepted",
        room_id: notificationCore.room_id,
    ...
    if (acceptNotificationError) {
      console.error("[Notifications Accept] Error sending acceptance notification:", acceptNotificationError.message);
    } else {
      console.log(`[Notifications Accept] Sent acceptance notification to user ${notificationCore.sender_id}`);
    }
    ...
    return NextResponse.json({ message: "Request accepted", notification: transformedNotification }, { status: 200 });
  } catch (error) {
