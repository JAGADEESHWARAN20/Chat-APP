...
export async function POST(req: NextRequest, { params }: { params: { userId?: string } }) {
  try {
    ...
    try {
      ...
      const completion = await withTimeout(completionPromise as Promise<any>, ABORT_MS + 1000);
      aiContent = parseContent(completion?.choices?.[0]?.message?.content ?? "");
    } catch (aiErr: any) {
    ...
    try {
      const insertTask = Promise.resolve(supabase.from("ai_chat_history").insert(insertData));
      await withTimeout(insertTask, 3000);
    } catch (dbErr) {
      console.warn("⚠️ DB insert slow/failed:", dbErr);
    }
