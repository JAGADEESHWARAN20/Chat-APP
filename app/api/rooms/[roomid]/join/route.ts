...
export async function POST(
  ...
  // Extract roomid correctly (lowercase 'id' from [roomid])
  const roomId = params.roomid?.trim();

  console.log('🔍 API ENTRY - Full Request URL:', request.url);
  console.log('🔍 API ENTRY - Raw Params Object:', params);
  console.log('🔍 API ENTRY - Extracted roomId:', roomId, '(type:', typeof roomId, ')');
  ...
  try {
    ...
    if (!roomId || roomId === 'undefined' || !UUID_REGEX.test(roomId)) {
      console.error('🚫 Validation Failed - roomId Details:', {
        roomId,
        isFalsy: !roomId,
        isUndefinedStr: roomId === 'undefined',
        regexMatch: UUID_REGEX.test(roomId || ''),
        fullParams: params,
      });
      return NextResponse.json(
        { error: 'Invalid or missing room ID', debug: { roomId, params } },
        { status: 400 }
      );
    }
    ...
    const senderName =
      senderProfile?.display_name || senderProfile?.username || session.user.email || 'A user';
