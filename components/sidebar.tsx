    ...
    const setOpen = React.useCallback(
      (value: boolean | ((value: boolean) => boolean)) => {
        ...
        try {
          ...
          // ignore cookie set errors in strict environments
        }
      },
      [setOpenProp, open]
    )
...
>(
  (
    ...
    // when collapsible disabled, render a simple fixed-width sidebar using CSS var
    if (collapsible === "none") {
      return (
        <div
          className={cn(
            "flex h-full",
            "w-[var(--sidebar-width)] flex-col",
            // use CSS var for background and text color
      ...
      return (
        <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
          <SheetContent
            ...
            data-mobile="true"
            // width via css var, override value for mobile with inline style below
            className={cn(
            ...
            style={
              {
                // override --sidebar-width for the sheet only
    ...
    // desktop / tablet interactive sidebar
    return (
      <div
        ref={ref}
        className={cn(
        ...
        data-side={side}
      >
        {/* invisible spacer for transitions */}
        <div
          className={cn(
        ...
        {/* Fixed interactive panel */}
        <div
          ref={ref}
          className={cn(
            // use CSS variables for width and z
            "duration-200 fixed inset-y-0 z-[var(--z-ui,10)] hidden h-svh transition-[left,right,width] ease-linear md:flex pointer-events-auto",
            "w-[var(--sidebar-width)]",
            side === "left"
        ...
        >
          <div
            data-sidebar="sidebar"
            // outer fixed container is transparent so theme wrapper controls background.
            className={cn(
            ...
            style={
              {
                ...
                // ensure ring color references var
                outlineColor: "hsl(var(--sidebar-ring))",
              } as React.CSSProperties
            }
          >
            {children}
...
>(({ className, ...props }, ref) => {
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn(
        "h-8 w-full bg-[var(--background)] shadow-none focus-visible:ring-2",
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        // slightly muted label color
        "text-[hsl(var(--sidebar-foreground))]/70",
        "outline-none ring-sidebar-ring transition-[margin,opacity] ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "absolute right-3 top-3.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 outline-none transition-transform",
        "text-[hsl(var(--sidebar-foreground))]",
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none transition-[width,height,padding] focus-visible:ring-2",
  {
    variants: {
      variant: {
        default: "hover:bg-[hsl(var(--sidebar-accent))] hover:text-[hsl(var(--sidebar-accent-foreground))]",
        outline:
          "bg-[var(--background)] shadow-[0_0_0_1px_hsl(var(--sidebar-border))] hover:bg-[hsl(var(--sidebar-accent))] hover:text-[hsl(var(--sidebar-accent-foreground))]",
      },
      size: {
        default: "h-8 text-sm",
        sm: "h-7 text-xs",
